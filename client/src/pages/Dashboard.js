...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (!user) {
      ...
      fetch("https://task-manager-backend-fzq7.onrender.com/api/tasks", {
        headers: {
  ...
  const updateTask = (updatedTask) => {
    ...
    fetch(`https://task-manager-backend-fzq7.onrender.com/api/tasks/${updatedTask.id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const deleteTask = (taskId) => {
    ...
    fetch(`https://task-manager-backend-fzq7.onrender.com/api/tasks/${taskId}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
